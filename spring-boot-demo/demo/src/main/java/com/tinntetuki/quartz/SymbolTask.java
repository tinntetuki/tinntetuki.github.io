...
import com.tinntetuki.service.SymbolServiceImpl;
import com.tinntetuki.util.HuoBiUtil;
import com.tinntetuki.util.OkHttpUtil;
...
public class SymbolTask{
    @Autowired
    SymbolServiceImpl symbolService;

    @Scheduled(fixedRate = 24*60*60*1000)
    protected void executeInternal() {
        try {
            Symbol symbol = HuoBiUtil.getSymbol();
            try {
                symbolService.batchAdd(symbol.getData());
            } catch (Exception e) {
